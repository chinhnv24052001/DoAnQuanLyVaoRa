...
@Component({
  selector: 'app-list-workers-in',
  templateUrl: './list-workers-in.component.html',
  styleUrls: ['./list-workers-in.component.less']
})
export class ListWorkersInComponent extends AppComponentBase implements AfterViewInit {
  ...
  getEmployeesIn(event?: LazyLoadEvent) {
    this.isLoading = true;
  ...
  bringModal(id, stringInOut) {
    this.message.confirm('', this.l(AppConsts.message.confimDelete), (isConfirmed) => {
      if (isConfirmed) {
        this.isLoading = true;
        this._assetInOutManamentServiceProxy.checkInOutEmployees(id, 0, stringInOut).subscribe((result) => {
